...
def submatrix(matrix, r_loc, c_loc):
    ...
    (c1, c2) = c_loc
    rows = matrix[r1:r2]
    submats = [row[c1:c2] for row in rows]
    return submats
...
def same_bits(bin_mat):
    ...
    bin_mat_flat = flatten(bin_mat)
    eq = None
    return [eq = (bin_mat_flat(0) == bin_mat_flat(n) for n in range(len(0, bin_mat_flat)))]
...
def stitch_horizontal(left, right):
    ...
    return list(map(lambda r1, r2 : r1+r2, left, right))
